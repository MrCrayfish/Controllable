...
public class ReiSupport
{
    ...
    @SuppressWarnings("UnstableApiUsage")
    private static boolean isValidWidgetType(GuiEventListener listener)
    {
        if(listener instanceof Button)
            return true;
        if(listener instanceof EntryListStackEntry)
            return true;
        if(listener instanceof Label label && label.isClickable())
            return true;
        if(listener instanceof TabWidget tabWidget && tabWidget.isShown() && !tabWidget.bounds.isEmpty() && tabWidget.opacity > 0)
            return true;
        if(listener instanceof Slot slot && slot.isInteractable())
            return true;
        return false;
    }
