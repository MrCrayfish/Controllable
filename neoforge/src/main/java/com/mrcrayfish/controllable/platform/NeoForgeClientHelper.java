...
import com.mrcrayfish.controllable.client.NeoForgeCompatBindingContext;
...
public class NeoForgeClientHelper implements IClientHelper
{
    ...
    public boolean sendScreenInput(Screen screen, int key, int action, int modifiers)
    {
        ...
        Screen.wrapScreenError(() -> {
            if(action == GLFW.GLFW_RELEASE) {
                cancelled[0] = ClientHooks.onScreenKeyReleasedPre(screen, key, -1, modifiers);
                if(!cancelled[0]) cancelled[0] = screen.keyReleased(key, -1, modifiers);
                if(!cancelled[0]) cancelled[0] = ClientHooks.onScreenKeyReleasedPost(screen, key, -1, modifiers);
            } else if(action == GLFW.GLFW_PRESS) {
                cancelled[0] = ClientHooks.onScreenKeyPressedPre(screen, key, -1, modifiers);
                if(!cancelled[0])  cancelled[0] = screen.keyPressed(key, -1, modifiers);
                if(!cancelled[0]) cancelled[0] = ClientHooks.onScreenKeyPressedPost(screen, key, -1, modifiers);
            }
    ...
    public void sendScreenMouseClickPre(Screen screen, double mouseX, double mouseY, int button)
    {
        Screen.wrapScreenError(() -> {
            ...
            if(!cancelled) {
                ...
                ClientHooks.onScreenMouseClickedPost(screen, mouseX, mouseY, button, cancelled);
            }
    ...
    public void sendScreenMouseReleasedPre(Screen screen, double mouseX, double mouseY, int button)
    {
        Screen.wrapScreenError(() -> {
            ...
            if(!cancelled) {
                ...
                ClientHooks.onScreenMouseReleasedPost(screen, mouseX, mouseY, button, cancelled);
            }
    ...
    {
        return this.keyContextMap.computeIfAbsent(mapping.getKeyConflictContext(), NeoForgeCompatBindingContext::new);
    }
